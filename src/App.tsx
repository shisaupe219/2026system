...
export default function App() {
  ...
  const guideSteps = [
    ...
    {
      title: "第一步：填写课程信息",
      content: "在这里输入课程的基本信息，或者点击“上传教学大纲”让 AI 自动为您提取信息。",
        ...
        <section id="section-course-info" className={cn("bg-white p-8 rounded-2xl shadow-sm border border-slate-200", isExporting && "p-0 border-none shadow-none")}>
          <SectionHeader icon={Info} title="一、课程基本信息" step={1}>
            <div className="flex gap-2">
              <label className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg cursor-pointer hover:bg-blue-100 transition-colors text-sm font-medium border border-blue-200">
                {isParsingSyllabus ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
                  <Upload className="w-4 h-4" />
                )}
                {isParsingSyllabus ? '解析中...' : '上传教学大纲'}
                <input 
                  type="file" 
                  className="hidden" 
